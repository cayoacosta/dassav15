...
class Landed_cost_inherit(models.Model):
    ...
    def get_valuation_lines(self):
        ...
        for move in self.mapped('picking_ids').mapped('move_lines'):
            ...
            if move.product_id.categ_id.property_valuation != 'real_time' or move.product_id.categ_id.property_cost_method not in  ['fifo','average']:
                continue
            vals = {
        ...
        for cost in self :
            for line in cost.valuation_adjustment_lines.filtered(lambda line: line.move_id):
                if line.move_id.picking_id.picking_type_id.code == 'incoming' :
                    ...
                    quantity_to_add = line.product_id.qty_available
                    if line.quantity > 0 :
                    ...
                    if line.product_id.cost_method == 'average'  and not float_is_zero(quantity_to_add, precision_rounding=line.product_id.uom_id.rounding):
                                line.product_id.with_company(self.company_id.id).sudo().standard_price += cost_to_add/quantity_to_add
