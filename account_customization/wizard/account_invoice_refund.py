...
class AccountInvoiceRefund(models.TransientModel):
    ...
    def _get_default_tipo(self):
        ...
        if ctx.get('active_model') == 'account.move':
            return self.env['account.move'].browse(ctx.get('active_ids')[0]).tipo

    @api.model
    def _get_default_journal(self):
        ctx = self._context
        if ctx.get('active_model') == 'account.move':
            return self.env['account.move'].browse(ctx.get('active_ids')[0]).journal_id.id

    @api.model
    def _get_default_account_id(self):
        ctx = self._context
        if ctx.get('active_model') == 'account.move':
            return self.env['account.move'].browse(ctx.get('active_ids')[0]).account_id_aux_refund.id
    ...
    nota_dev = fields.Selection([('devolucion','Devolución'),('bonificacion','Bonificación')],
